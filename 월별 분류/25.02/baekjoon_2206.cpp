...
int N, M;
int arr[1001][1001] = { 0, };
int min_data_0[1001][1001] = { 0, };
int min_data_1[1001][1001] = { 0, };
int dy[4] = {0, 1, 0, -1};
int dx[4] = {1, 0, -1, 0};
int ans = -1;
...
void cal() {
	...
	while (!q.empty()) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (arr[ny][nx] == 1 && n_check == 1) continue;
			else if (arr[ny][nx] == 1 && n_check == 0) {
				if (min_data_1[ny][nx] <= n_cnt) continue;
				n_check = 1;
				if (min_data_1[ny][nx] > n_cnt) min_data_1[ny][nx] = n_cnt;
			}
			else if (arr[ny][nx] == 0 && n_check == 1){
				if (min_data_1[ny][nx] <= n_cnt) continue;
				if (min_data_1[ny][nx] > n_cnt) min_data_1[ny][nx] = n_cnt;
			}
			else {
				if (min_data_0[ny][nx] <= n_cnt) continue;
				if (min_data_0[ny][nx] > n_cnt) min_data_0[ny][nx] = n_cnt;
			}
